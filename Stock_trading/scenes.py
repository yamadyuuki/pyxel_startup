...
class PlayScene(BaseScene):
    ...
    def on_enter(self, **kwargs):
        ...
        self.day = 1
        self.cursor_pos = 0 # 0:買う, 1:売る, 2:次の日へ
        ...
        # 銘柄を作成するときに、どの戦略を使うか指定して渡す
        self.market.add_stock(Stock(name="Pixel Inc.", initial_price=5000, strategy=random_strategy))
        self.market.add_stock(Stock(name="Dot Foods", initial_price=3000, strategy=trending_strategy))        # self.market.add_stock(Stock(name="Dot Foods", initial_price=3000)) # 銘柄は簡単に追加できる
    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.KEY_UP):
            self.cursor_pos = (self.cursor_pos - 1 + 3) % 3
            
        if pyxel.btnp(pyxel.KEY_SPACE):
            stock = self.market.stocks["Pixel Inc."] # 今は1銘柄のみ対象
            
            if self.cursor_pos == 0: # 買う
                self.player.buy(stock.name, 1, stock.current_price)
            elif self.cursor_pos == 1: # 売る
                self.player.sell(stock.name, 1, stock.current_price)
            elif self.cursor_pos == 2: # 次の日へ
    ...
    def draw(self):
        ...
        # --- 銘柄情報 ---
        stock = self.market.stocks["Pixel Inc."]
        pyxel.text(5, 30, f"STOCK: {stock.name}", 7)
        ...
        for i in range(1, len(history)):
            pyxel.line(5 + i * multiple - multiple, 150 - history[i-1] // 100, 5 + i * multiple, 150 - history[i] // 100, pyxel.COLOR_GREEN)

        # --- メニュー ---
        pyxel.text(100, 150, "BUY (1 stock)", 7)
        pyxel.text(100, 160, "SELL (1 stock)", 7)
        pyxel.text(100, 170, "NEXT DAY", 7)
        pyxel.text(90, 150 + self.cursor_pos * 10, ">", pyxel.COLOR_YELLOW)
...
class ResultScene(BaseScene):
    ...
    def draw(self):
        ...
        pyxel.text(90, 120, "- PRESS ENTER -", 7)